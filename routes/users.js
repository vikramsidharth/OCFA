...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO users (
        username, password, name, role, email, unit, category, "MobileNumber", "EmployeeID"
      ) VALUES (
        $1, $2, $3, $4, $5, $6, $7, $8, $9
      ) RETURNING id, username, name, role, email, unit, category, "MobileNumber", "EmployeeID"`,
      [
        username,
        hashedPassword,
        name,
...
// POST /api/move-location
router.post('/move-location', async (req, res) => {
  const { userId, latitude, longitude, heading, timestamp } = req.body;
  if (!userId || typeof latitude !== 'number' || typeof longitude !== 'number') {
    return res.status(400).json({ error: 'userId, latitude, and longitude are required and must be valid' });
  }
  try {
    // Insert into locations table (create if not exists)
    await pool.query(
    ...
    const result = await pool.query(
      'INSERT INTO locations (user_id, latitude, longitude, heading, updated_at) VALUES ($1, $2, $3, $4, to_timestamp($5 / 1000.0)) RETURNING *',
      [userId, latitude, longitude, heading, timestamp || Date.now()]
    );
    res.status(201).json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
